...
/* ==========================================================================================
   LOCKED 2026-08-06 — THE STRAND'S MOTION IS SIGNED OFF. DO NOT RETUNE IT.
   Damir watched it and said: "dna background is moving perfect now lock it this way bro
   important so no more fuck ups."

   That means RUNWAY, FPS, HALF_FRAME, SEEK_MS, EASE, the SRC_* ladder and the rAF scrub loop
   below are closed. Not "closed unless you have a good reason" - closed. Every one of these
   numbers is the end of an argument that was already had and already lost once.

   A brief that says the header is slow, long, jumpy or heavy is NOT permission to touch them:
...
/* THE DEMOS, in ONE place. This list was pasted inline into three page bodies, which is how a
...
const DEMOS = [
  ...
  /* v2, not the client's build. demo/vzaimoraschety/ is the copy delivered against the signed
     technical proposal and is not ours to restyle - it stays deployed at its own URL, untouched.
     The card shows the restyled twin. */
  ['vzaimoraschety-v2', 'Взаиморасчёты с контрагентами', 'Свёрнутое сальдо по каждому контрагенту, сроки долга, акт сверки одной кнопкой. Правила свёртки настраиваются под вашу учётную политику.'],
  ['sklad', 'Складской терминал', 'Отбор по FEFO, приёмка, размещение, перемещение, слепой пересчёт. Терминал не даёт взять не ту партию, а расхождения сразу видны в офисе.'],
] as const
...
export default function V2() {
  ...
  return (
    <>
      ...
      {/* THE SKY TOGGLE IS GONE. His call, looking at it: the sun/moon pill goes, the 3D CTA
          takes its corner. Nothing calls switchTheme() any more, so the light theme is
          unreachable - its machinery below and its six rules in v2.css are left standing
          because they cost nothing dormant and putting a trigger back is one line. Say the
          word and they come out for real.
          ponytail: dead but load-bearing-if-he-changes-his-mind. */}
      ...
      <header className="v2nav">
        ...
        {/* ONE CTA IN THE BAR, NOT TWO. The 3D button takes the corner the sky toggle had, and
            the flat white pill that used to sit beside it is gone - two "оставить заявку"
            buttons 40px apart is not emphasis, it is a page that cannot decide. */}
        <Btn3D className="v2nav__cta" href="#/contacts"
               label="Оставить заявку" hover="Обсудить задачу" />
      </header>
